...
public class WarriorAnimationActions : PlayerAnimationActions
{
    ...
    {
        base.AttackHit();
        Collider2D col = Physics2D.OverlapBox((Vector2)player.transform.position + new Vector2(_attackCastOffset.x * _movement.direction,
                                                                                        _attackCastOffset.y), 
                                               _attackCastSize, 0.0f, _attackTargetMask);
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(playerPos + new Vector3(_attackCastOffset.x * direction, _attackCastOffset.y, 0.0f),
                                                             _attackCastSize);
    }
}
