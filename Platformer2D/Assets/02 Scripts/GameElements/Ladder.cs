...
public class Ladder : MonoBehaviour
{
    public Vector2 ladderUpStartPos => (Vector2)transform.position + 
                                        Vector2.down * _bound.size.y / 2.0f + Vector2.up * _ladderUpStartOffsetY;
    public Vector2 ladderUpEndPos => (Vector2)transform.position + 
                                      Vector2.up * _bound.size.y / 2.0f + Vector2.down * _ladderUpEndOffsetY;

    public Vector2 ladderDownStartPos => (Vector2)transform.position +
                                          Vector2.up * _bound.size.y / 2.0f + Vector2.down * _ladderDownStartOffsetY;
    public Vector2 ladderDownEndPos => (Vector2)transform.position +
                                        Vector2.down * _bound.size.y / 2.0f + Vector2.down * _ladderDownEndOffsetY;
